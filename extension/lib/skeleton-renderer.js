 ...
 * ====================================================================
 * Ported from the DuoSign frontend TypeScript to vanilla JS for extension use.
 *
...
function drawComponent(ctx, component, points, scale, lineWidth) {
  ...
  for (let i = 0; i < component.limbs.length; i++) {
    ...
    if (!from || !to) continue;
    if ((from.C ?? 0) < MIN_CONFIDENCE || (to.C ?? 0) < MIN_CONFIDENCE)
      continue;
...
function buildNameMap(component) {
  const map = new Map();
...
function featurePts(indices, nameMap, points, scale) {
  return indices
    .map((i) => {
...
function drawFaceMask(ctx, component, points, scale) {
  if (!points || points.length === 0) return;

  const nm = buildNameMap(component);
  const get = (indices) => featurePts(indices, nm, points, scale);
...
/**
 * Draw a single frame of pose data onto a canvas.
 * @param {CanvasRenderingContext2D} ctx
 * @param {Object} frame — { people: [{ COMPONENT_NAME: [{X, Y, C}, ...], ... }] }
 * @param {Object} header — { width, height, components: [...] }
 * @param {number} canvasWidth
 * @param {number} canvasHeight
 */
function drawSkeleton(ctx, frame, header, canvasWidth, canvasHeight) {
...
// Make available globally (for script tag loading in extension)
